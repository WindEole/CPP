...
/*   Updated: 2022/06/13 15:18:58 by iderighe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::ClapTrap(std::string name, int HP, int EP, int AD)
{
	...
	std::cout << "ClapTrap -> ScavTrap constructor called : " << this->_name << " is created." << std::endl;	
	return;
}
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	...
	{
		std::cout << "[" << this->_name << "] doesn't need to be repaired. Don't waste your Energy Points ! EP = " << this->_EnergyPoints << std::endl;
		return ;
	}
	this->_HitPoints += amount;
	if (this->_HitPoints >= this->_initialHP)
	{
		...
		std::cout << "[" << this->_name << "] is totally repaired ! He has " << this->_HitPoints << " Hit Points now. EP = " << this->_EnergyPoints << std::endl;
	}
	else
	{
		std::cout << "[" << this->_name << "] has recovered " << amount << " Hit Points !";
		std::cout << " He has " << this->_HitPoints << " Hit Points now. EP = " << this->_EnergyPoints << std::endl;			
	}
}
