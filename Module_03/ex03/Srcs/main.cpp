...
int	main(void)
{
	...
	// }
	{
		std::cout << std::endl << RED << "test DiamondTrap" << RESET << std::endl;
	
		ClapTrap	Conan("Conan");
		ScavTrap	Dovahkiin("Dovahkiin");
		FragTrap	Hulk("Hulk");
		DiamondTrap	WonderWoman("WonderWoman");

		std::cout << std::endl << YELLOW << "test ClapTrap" << RESET << std::endl;
		Conan.attack(Dovahkiin.getName());
		Conan.takeDamage(5);
		Conan.beRepaired(2);
		Conan.takeDamage(10);
		
		std::cout << std::endl << BLUE << "test ScavTrap" << RESET << std::endl;
		Dovahkiin.attack(Hulk.getName());
		Dovahkiin.takeDamage(50);
		Dovahkiin.beRepaired(20);
		Dovahkiin.guardGate();
		Dovahkiin.takeDamage(130);

		std::cout << std::endl << CYAN << "test FragTrap" << RESET << std::endl;
		Hulk.attack(WonderWoman.getName());
		Hulk.takeDamage(50);
		Hulk.beRepaired(20);
		Hulk.highFivesGuys();
		Hulk.takeDamage(130);

		std::cout << std::endl << GREEN << "test DiamondTrap" << RESET << std::endl;
		WonderWoman.whoAmI();
		WonderWoman.attack(Dovahkiin.getName());
		WonderWoman.takeDamage(30);
		WonderWoman.beRepaired(15);
		WonderWoman.whoAmI();
		WonderWoman.highFivesGuys();
		WonderWoman.guardGate();
		WonderWoman.takeDamage(130);
		std::cout << std::endl;
	}
	return (0);
}
